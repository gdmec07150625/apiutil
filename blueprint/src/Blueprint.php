...
class Blueprint
{
    ...
    {
        $this->reader->addNamespace('Apiutil\\Blueprint\\Annotation');
        $this->reader->addNamespace('Apiutil\\Blueprint\\Annotation\\Method');

        AnnotationRegistry::registerLoader(function ($class) {
            $path = __DIR__.str_replace(['Apiutil\\Blueprint', '\\'], ['', DIRECTORY_SEPARATOR], $class).'.php';
            if (file_exists($path)) {
                require_once $path;
    ...
    protected function  generateContentsFromResources(Collection $resources, $type, $overviewDir = null)
    {
        ...
        $resources->each(function ($resource) use ($writer, $overviewDir) {
            ...
            $resource->getActions()->each(function ($action) use ($resource, $writer, $overviewDir) {
                ...
                if ($authors = $action->getAuthors()) {
                    $this->appendAuthors($contents, $authors);
                }
    ...
    public function appendAuthors (&$contents, $authors) {
        $contents .= $this->line();
        $contents .= sprintf('+ Authors');
        foreach ($authors->value as $author) {
            $contents .= $this->line();
            $contents .= $this->tab();
            $contents .= sprintf('+ Author:%s', $author);
        }
     ...
     *
     * @param string                               $contents
     * @param \Apiutil\Blueprint\Annotation\Response $response
     * @param \Apiutil\Blueprint\RestResource            $resource
     *
     * @return void
     */
     ...
     *
     * @param string                              $contents
     * @param \Apiutil\Blueprint\Annotation\Request $request
     * @param \Apiutil\Blueprint\RestResource           $resource
     *
     * @return void
     */
